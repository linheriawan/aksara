<script lang="ts">
import "./style.css"
import type {FormElement,Schema} from "$lib/core/schema"
import type {DraggedItem,DropZone} from "./utils.ts"
...
// Form-specific state
let formName = $state(data.formData?.name || 'Untitled Form');
let formDescription = $state(data.formData?.description || '');
let isSaving = $state(false);
let hasUnsavedChanges = $state(false);

// Load form data if available
$effect(() => {
  if (data.formData?.schema) {
    schemaManager.setSchema(data.formData.schema);
  }
...
const exportYaml = () => {
  ...
  a.href = url;
  a.download = `${formName.replace(/[^a-z0-9]/gi, '-').toLowerCase()}-schema.yaml`;
  a.click();
...
<div class="flex justify-between items-center pt-1 mb-2">
  ...
  <div class="flex items-center space-x-2">
    <button 
      ...
      class="btn border rounded-md px-3 py-1 text-sm bg-blue-600 text-white hover:bg-blue-700 disabled:bg-gray-300">
      {isSaving ? '⏳ Saving...' : hasUnsavedChanges ? '💾 Save' : '✓ Saved'}
    </button>
    <button onclick={exportYaml} class="btn border rounded-md p-1 text-xs">📤 YAML</button>
    <button onclick={exportJson} class="btn border rounded-md p-1 text-xs">📤 JSON</button>
  </div>
</div>
<hr class="mb-1">
