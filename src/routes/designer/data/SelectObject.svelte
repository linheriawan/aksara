...
const dataSourceObjects = $derived(() => {
  if (!dataSourceConfig) return [];
  return objects.filter(obj => obj.dataSource === dataSourceConfig.name);
});
...
<div class="space-y-6">
  <div>
    <h2 class="text-2xl font-semibold mb-2">Select or Create Object</h2>
    <p class="text-gray-600">Choose an existing object to edit or create a new one to map to your data source.</p>
  </div>

  <!-- Existing Objects List -->
  <div>
    <h3 class="text-lg font-medium mb-4">
      Existing Objects for {dataSourceConfig?.name} ({dataSourceObjects.length})
    </h3>
    
    {#if dataSourceObjects.length > 0}
      <div class="grid gap-4 mb-8">
        {#each dataSourceObjects as object, index}
          <div class="border border-gray-300 rounded-lg p-4 hover:bg-gray-50 transition-colors cursor-pointer"
               onclick={() => handleSelectObject(object, index)}>
            <div class="flex justify-between items-start">
              <div class="flex-1">
                <h4 class="font-medium text-lg text-gray-900">{object.name}</h4>
                <div class="mt-2 space-y-1 text-sm text-gray-600">
                  <p><span class="font-medium">Source:</span> {object.source}</p>
                  <p><span class="font-medium">Fields:</span> {object.fields.length}</p>
                  {#if object.primaryKey}
                    <p><span class="font-medium">Primary Key:</span> {object.primaryKey}</p>
                  {/if}
                  <div class="flex items-center mt-2">
                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                      {dataSourceConfig?.type.toUpperCase()}
                    </span>
                  </div>
                </div>
              </div>
    ...
    {:else}
      <div class="text-center py-8 text-gray-500 border-2 border-dashed border-gray-300 rounded-lg mb-8">
        ...
        <p class="text-lg font-medium">No objects for this data source</p>
        <p class="mt-1">Create your first object for <strong>{dataSourceConfig?.name}</strong></p>
      </div>
    {/if}
  </div>
  ...
  <div class="border-t pt-6">
    <h3 class="text-lg font-medium mb-4">Create New Object</h3>
    ...
    <div class="bg-gray-50 rounded-lg p-6 space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          />
          <p class="mt-1 text-xs text-gray-500">Choose a descriptive name for your object</p>
        </div>
        ...
        <div>
          ...
          <p class="mt-1 text-xs text-gray-500">Choose the source table, endpoint, or file</p>
        </div>
      </div>
      ...
      <div class="flex justify-end">
        ...
        >
          ...
          <span>Create & Configure Object</span>
        </button>
      </div>
    </div>
  </div>
