<!-- src/routes/designer/data/wizard/+page.svelte -->
<script lang="ts">
...
// URL parameters
let dataSourceParam = $state('');
let objectParam = $state('');
let mode = $state<'create' | 'edit' | 'editSource'>('create');
...
onMount(async () => {
  ...
  dataSourceParam = urlParams.get('dataSource') || '';
  objectParam = urlParams.get('object') || '';
  mode = (urlParams.get('mode') as 'create' | 'edit' | 'editSource') || 'create';
  ...
  try {
    ...
    // Handle different modes based on URL parameters
    if (mode === 'editSource' && dataSourceParam) {
      // Edit existing data source
      const existingDataSource = dataSources.find(ds => ds.name === dataSourceParam);
      if (existingDataSource) {
        dataSourceConfig = existingDataSource;
        currentStep = 1; // Stay on data source step for editing
      }
    } else if (mode === 'edit' && dataSourceParam && objectParam) {
      // Edit existing object schema
      const existingDataSource = dataSources.find(ds => ds.name === dataSourceParam);
      const existingObject = objects.find(obj => obj.name === objectParam && obj.dataSource === dataSourceParam);
      
      if (existingDataSource && existingObject) {
        dataSourceConfig = existingDataSource;
        currentObject = existingObject;
        currentStep = 3; // Jump to field mapping step
        
        // Load objects for this data source
        const objectResponse = await fetch(`/designer/data/load-configs?dataSource=${dataSourceParam}`);
        if (objectResponse.ok) {
          const objectData = await objectResponse.json();
          objects = objectData.objects || [];
        }
      }
    } else if (mode === 'create' && dataSourceParam) {
      // Create new object for existing data source
      const existingDataSource = dataSources.find(ds => ds.name === dataSourceParam);
      if (existingDataSource) {
        dataSourceConfig = existingDataSource;
        currentStep = 2; // Jump to object selection step
    ...
    // If no valid parameters, start fresh wizard (currentStep = 1)
